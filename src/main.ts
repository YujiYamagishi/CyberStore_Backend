...
import { ListAllProductsRoute } from './infra/api/express/routes/product/list-all.express';
...
process.on('unhandledRejection', (reason, promise) => { console.error('❌ ERRO FATAL: Rejeição não tratada.', { promise, reason }); process.exit(1); });
process.on('uncaughtException', (error) => { console.error('❌ ERRO FATAL: Exceção não capturada.', error); process.exit(1); });
...
function main() {
    ...
    const listAllProductsRoute = new ListAllProductsRoute();
